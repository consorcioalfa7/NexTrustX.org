...
export function PaymentModal({ isOpen, onClose, product }: PaymentModalProps) {
  ...
  const getPixLink = () => {
    ...
    // Converter para string e substituir vírgula por ponto para o link PIX
            ...
            {formData.paymentMethod === 'pix' ? (
              ...
              <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Valor a pagar: <span className="text-green-400 font-semibold">${(parseFloat(String(product?.price || product?.preco || '0')) / 5.5).toFixed(2)} USDT</span> (aproximado)
                </p>
              </div>
            )}
